...
def chain(chaindb, funded_address, funded_address_initial_balance):  # noqa: F811
    ...
    klass = Chain.configure(
        __name__='TestChain',
        vm_configuration=((constants.GENESIS_BLOCK_NUMBER, SpuriousDragonVM),),
    )
...
PAYGAS_contracts = json.load(
    open(
        os.path.join(
            os.path.dirname(__file__), './contract_fixtures/PAYGAS_contracts.json'
        )
    )
)
CREATE2_contracts = json.load(
    open(
        os.path.join(
            os.path.dirname(__file__), './contract_fixtures/CREATE2_contracts.json'
        )
    )
)
nonce_tracking_contracts = json.load(
    open(
        os.path.join(
            ...
            './contract_fixtures/nonce_tracking_contracts.json',
        )
    )
)
SHARD_CHAIN_CONTRACTS_FIXTURES = [
    ...
    {
        "contract_code": PAYGAS_contracts['PAYGAS_contract_triggered_twice'][
            'bytecode'
        ],
        "deployed_address": PAYGAS_contracts['PAYGAS_contract_triggered_twice'][
            'address'
        ],
        "initial_balance": funded_address_initial_balance(),
    },
    {
        "contract_code": nonce_tracking_contracts['nonce_tracking_contract'][
            'bytecode'
        ],
        "deployed_address": nonce_tracking_contracts['nonce_tracking_contract'][
            'address'
        ],
        "initial_balance": funded_address_initial_balance(),
    },
    {
        "contract_code": nonce_tracking_contracts['no_nonce_tracking_contract'][
            'bytecode'
        ],
        "deployed_address": nonce_tracking_contracts['no_nonce_tracking_contract'][
            'address'
        ],
        "initial_balance": funded_address_initial_balance(),
    },
...
@pytest.fixture
def shard_chain(
    shard_chaindb, funded_address, funded_address_initial_balance
):  # noqa: F811
    genesis_params = {
        "bloom": 0,
        "coinbase": to_canonical_address("8888f1f195afa192cfee860698584c030f4c9db1"),
...
def chain_without_block_validation(
    chaindb, funded_address, funded_address_initial_balance
):  # noqa: F811
    """
    Return a Chain object containing just the genesis block.
