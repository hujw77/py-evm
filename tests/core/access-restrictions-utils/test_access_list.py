...
from evm.constants import (STORAGE_TRIE_PREFIX,)
from evm.validation import (validate_transaction_access_list,)

from evm.rlp.sedes import (access_list as access_list_sedes,)
from evm.utils.state_access_restriction import (
    is_accessible, remove_redundant_prefixes, to_prefix_list_form
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        [
            to_prefix_list_form([[TEST_ADDRESS1, b'']]),
            TEST_ADDRESS1,
            b'\x00' * 32,
            True
        ],
...
@pytest.mark.parametrize(
    ...
    (
        ((b'', b'something'), {b''}),
        ((b'ethereum', b'eth', b'ether', b'england', b'eng'), {b'eth', b'eng'}),
        ((b'ethereum', b'ethereua'), {b'ethereum', b'ethereua'}),
        ((b'a', b'aa', b'b', b'bb', b'ab', b'ba'), {b'a', b'b'}),
    ),
)
...
@pytest.mark.parametrize(
    ...
    [
        ((), b'\xc0'),
        (((TEST_ADDRESS1,),), b'\xd6' + b'\xd5' + b'\x94' + TEST_ADDRESS1),
        (
            ((TEST_ADDRESS1, b''),),
        ...
        (
            ...
            (
                ...
                b'\xd8' +
                (b'\x94' + TEST_ADDRESS2 + b'\x82\xaa\xbb')
            ),
        ),
    ],
)
