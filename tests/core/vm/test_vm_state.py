...
def test_apply_transaction(  # noqa: F811
    chain_without_block_validation, funded_address, funded_address_private_key
):
    chain = chain_without_block_validation  # noqa: F811
    ...
    tx1 = new_transaction(
        vm_example, from_, recipient1, amount, private_key=funded_address_private_key
    )
    ...
    tx2 = new_transaction(
        vm_example, from_, recipient2, amount, private_key=funded_address_private_key
    )
    ...
    prev_hashes = vm.get_prev_hashes(last_block_hash=prev_block_hash, db=vm.chaindb)
    ...
    computation, block, _ = vm_state1.apply_transaction(tx1, block1)
    ...
    computation, block, _ = vm_state1.apply_transaction(tx2, block)
    ...
    computation, block, _ = vm_state2.apply_transaction(tx1, block2)
    # Update witness_db
    ...
    computation, block, _ = vm_state2.apply_transaction(tx2, block)
    # After applying
