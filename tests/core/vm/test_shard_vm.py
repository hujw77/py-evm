...
from evm.exceptions import (IncorrectContractCreationAddress, ContractCreationCollision)
from evm.utils.address import generate_CREATE2_contract_address
from evm.utils.padding import pad32

from tests.core.helpers import (new_sharding_transaction,)
...
def test_sharding_apply_transaction(unvalidated_shard_chain):  # noqa: F811
    ...
    simple_factory_contract_bytecode = CREATE2_contracts["simple_factory_contract"][
        "bytecode"
    ]
...
def test_CREATE2_deploy_contract_edge_cases(unvalidated_shard_chain):  # noqa: F811
    ...
    first_failed_deploy_tx = new_sharding_transaction(
        ...
        code=code,
        access_list=[
            [decode_hex(simple_transfer_contract['address'])], [computed_address]
        ],
    )
    vm = chain.get_vm()
