...
from evm.utils.blobs import (calc_chunks_root, calc_merkle_root, chunk_iterator)
...
def test_chunk_iteration():
    ...
    test_chunks = [
        zpad_left(int_to_big_endian(i), CHUNK_SIZE) for i in range(chunk_number)
    ]
    chunks = test_chunks
    body = b"".join(chunks)
    for recovered, original in zip_longest(
        chunk_iterator(body), chunks, fillvalue=None
    ):
