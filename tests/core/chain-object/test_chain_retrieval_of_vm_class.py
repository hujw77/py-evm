...
from evm.chains.mainnet.constants import (HOMESTEAD_MAINNET_BLOCK)
from evm import Chain

from evm.exceptions import (ValidationError, VMNotFound)
...
def test_get_vm_class_for_block_number():
    ...
    assert chain.get_vm_class_for_block_number(
        HOMESTEAD_MAINNET_BLOCK - 1
    ) == FrontierVM
    assert chain.get_vm_class_for_block_number(HOMESTEAD_MAINNET_BLOCK) == HomesteadVM
    assert chain.get_vm_class_for_block_number(
        HOMESTEAD_MAINNET_BLOCK + 1
    ) == HomesteadVM
...
def test_vm_not_found_if_no_matching_block_number():
    chain_class = Chain.configure('TestChain', vm_configuration=((10, FrontierVM),))
    chain = chain_class(get_db_backend(), BlockHeader(1, 0, 100))
...
def test_configure_duplicate_block_numbers_in_vm_configuration():
    with pytest.raises(ValidationError):
        Chain.configure(
            'TestChain', vm_configuration=[(0, FrontierVM), (0, HomesteadVM)]
        )
