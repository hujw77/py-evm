...
from viper.compile_lll import (compile_to_assembly, assembly_to_evm)
...
from tests.core.contract_fixtures.nonce_tracking_contract import (
    nonce_tracking_lll_code, no_nonce_tracking_lll_code
)
...
simple_forwarder_contract_bytecode = assembly_to_evm(
    compile_to_assembly(LLLnode.from_list(simple_forwarder_contract_lll_code))
)
PAYGAS_contract_normal_bytecode = assembly_to_evm(
    compile_to_assembly(LLLnode.from_list(PAYGAS_contract_normal_lll_code))
)
PAYGAS_contract_triggered_twice_bytecode = assembly_to_evm(
    compile_to_assembly(LLLnode.from_list(PAYGAS_contract_triggered_twice_lll_code))
)
PAYGAS_json = {
    ...
    "PAYGAS_contract_triggered_twice": {
        ...
        "address": encode_hex(
            generate_CREATE2_contract_address(
                b'', PAYGAS_contract_triggered_twice_bytecode
            )
        ),
    },
}
...
nonce_tracking_contract_bytecode = assembly_to_evm(
    compile_to_assembly(LLLnode.from_list(nonce_tracking_lll_code))
)
no_nonce_tracking_contract_bytecode = assembly_to_evm(
    compile_to_assembly(LLLnode.from_list(no_nonce_tracking_lll_code))
)
nonce_tracking_json = {
