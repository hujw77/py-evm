...
async def get_directly_linked_peers_without_handshake(
    peer1_class=LESPeer, peer1_chaindb=None, peer2_class=LESPeer, peer2_chaindb=None
):
    ...
    peer2_writer = type(
        "mock-streamwriter",
        (object,),
        {"write": peer1_reader.feed_data, "close": lambda: None},
    )
    peer1_writer = type(
        "mock-streamwriter",
        (object,),
        {"write": peer2_reader.feed_data, "close": lambda: None},
    )
...
async def get_directly_linked_peers(
    ...
    peer1, peer2 = await get_directly_linked_peers_without_handshake(
        peer1_class, peer1_chaindb, peer2_class, peer2_chaindb
    )
