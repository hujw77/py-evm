...
from eth_utils import (decode_hex, keccak, to_bytes)
...
def test_ping_pong():
    ...
    received_pongs = []
    alice.recv_pong = lambda node, payload, hash_: received_pongs.append(
        (node, payload)
    )
...
def test_find_node_neighbours():
    ...
    received_neighbours = []
    alice.recv_neighbours = lambda node, payload, hash_: received_neighbours.append(
        (node, payload)
    )
...
eip8_packets = {
    discovery.CMD_PING: dict(ping1=decode_hex(remove_chars('''
    ...
    discovery.CMD_PONG: dict(pong=decode_hex(remove_chars('''
    ...
    discovery.CMD_FIND_NODE: dict(findnode=decode_hex(remove_chars('''
    ...
    discovery.CMD_NEIGHBOURS: dict(neighbours=decode_hex(remove_chars('''
