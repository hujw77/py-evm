...
def pytest_generate_tests(metafunc):
    generate_fixture_tests(
        ...
        filter_fn=filter_fixtures(
            fixtures_base_dir=BASE_FIXTURE_PATH, mark_fn=vm_fixture_mark_fn
        ),
    )
...
def fixture(fixture_data):
    ...
    fixture = load_fixture(fixture_path, fixture_key, normalize_vmtest_fixture)
    return fixture
...
HomesteadVMForTesting = HomesteadVM.configure(
    __name__='HomesteadVMForTesting', _state_class=HomesteadVMStateForTesting
)
...
def fixture_to_computation(fixture, code, vm):
    ...
    transaction_context = BaseTransactionContext(
        origin=fixture['exec']['origin'], gas_price=fixture['exec']['gasPrice']
    )
    return vm.state.get_computation(message, transaction_context).apply_computation(
        vm.state, message, transaction_context
    )
...
@pytest.mark.parametrize(
    'computation_getter', (fixture_to_bytecode_computation, fixture_to_computation)
)
