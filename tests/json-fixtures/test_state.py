...
def pytest_generate_tests(metafunc):
    generate_fixture_tests(
        ...
        filter_fn=filter_fixtures(
            fixtures_base_dir=BASE_FIXTURE_PATH, mark_fn=mark_statetest_fixtures
        ),
    )
...
def fixture(fixture_data):
    ...
    fixture = load_fixture(
        ...
        fixture_key,
        normalize_statetest_fixture(
            fork=fixture_fork, post_state_index=post_state_index
        ),
    )
    return fixture
...
FrontierVMStateForTesting = FrontierVMState.configure(
    __name__='FrontierVMStateForTesting', get_ancestor_hash=get_block_hash_for_testing
)
HomesteadVMStateForTesting = HomesteadVMState.configure(
    __name__='HomesteadVMStateForTesting', get_ancestor_hash=get_block_hash_for_testing
)
...
ByzantiumVMStateForTesting = ByzantiumVMState.configure(
    __name__='ByzantiumVMStateForTesting', get_ancestor_hash=get_block_hash_for_testing
)
ShardingVMStateForTesting = ShardingVMState.configure(
    __name__='ShardingVMStateForTesting', get_ancestor_hash=get_block_hash_for_testing
)
...
def test_state_fixtures(fixture, fixture_vm_class):
    ...
    chaindb = ChainDB(
        get_db_backend(), account_state_class=account_state_class, trie_class=trie_class
    )
    ...
    if not transaction_error:
        ...
        if 'logs' in fixture['post']:
            ...
            raise AssertionError(
                "Got log {0} entries. hash:{1}".format(
                    len(log_entries), actual_logs_hash
                )
            )
