...
from cytoolz import (merge, dissoc, assoc)

from evm.constants import (UINT_256_MAX, SECPK1_N, ZERO_HASH32, ENTRY_POINT, EMPTY_SHA3)
from evm.vm.message import (ShardingMessage,)
from evm.vm.forks.sharding import (ShardingVM,)
from evm.vm.forks.sharding.transaction_context import (ShardingTransactionContext,)
...
from evm.db.state import (ShardingAccountStateDB,)

from eth_utils import (
    keccak, to_canonical_address, int_to_big_endian, big_endian_to_int, decode_hex
)
from evm.utils.padding import (pad32, zpad_left)
...
GAS_LOGGING_CONTRACT_ADDRESS = generate_CREATE2_contract_address(
    b"", GAS_LOGGING_CONTRACT_CODE
)
...
DEFAULT_BASE_TX_PARAMS = {
    ...
    "to": ACCOUNT_ADDRESS,
    "gas": 500000,
    "access_list": [[ACCOUNT_ADDRESS, b"\x00" * 32], [ECRECOVER_ADDRESS]],
    "code": b"",
    "salt": b"\x00" * 32
}
DEFAULT_TX_PARAMS = merge(
    dissoc(DEFAULT_BASE_TX_PARAMS, "code", "salt"),
    {
        ...
        "nonce": 0,
        "msg_data": b"",
        "access_list": DEFAULT_BASE_TX_PARAMS["access_list"] + [[DESTINATION_ADDRESS]],
        "gas_price": 0,
    },
)
SIGNED_DEFAULT_TRANSACTION = UnsignedUserAccountTransaction(
    ** merge(DEFAULT_TX_PARAMS, {"nonce": 0})
...
def get_nonce(vm):
    computation, _ = vm.apply_transaction(
        ShardingTransaction(
            **
            merge(DEFAULT_BASE_TX_PARAMS, {"data": int_to_big_endian(NONCE_GETTER_ID)})
        )
    )
...
def test_call_increments_nonce(vm):
    ...
    transaction = UnsignedUserAccountTransaction(
        ** merge(DEFAULT_TX_PARAMS, {"nonce": 1})
    ).as_signed_transaction(
        PRIVATE_KEY
    )
...
def test_call_checks_nonce(vm):
    ...
    transaction = UnsignedUserAccountTransaction(
        ** merge(DEFAULT_TX_PARAMS, {"nonce": 2})
    ).as_signed_transaction(
        PRIVATE_KEY
    )
    computation, _ = vm.apply_transaction(transaction)
...
@pytest.mark.parametrize(
    "min_block,max_block,valid",
    [
        (min_block, max_block, True)
        for min_block, max_block in [
            (0, UINT_256_MAX), (0, 10), (10, 10), (10, UINT_256_MAX)
        ]
    ] +
    [
        (min_block, max_block, False)
        for min_block, max_block in [
            (0, 9), (5, 9), (11, 20), (11, UINT_256_MAX), (11, 9), (UINT_256_MAX, 0)
        ]
    ],
)
def test_call_checks_block_range(vm, min_block, max_block, valid):
    ...
    transaction = UnsignedUserAccountTransaction(
        **
        merge(
            DEFAULT_TX_PARAMS,
            {"nonce": get_nonce(vm), "min_block": min_block, "max_block": max_block},
        )
    ).as_signed_transaction(
        PRIVATE_KEY
    )
...
def test_call_transfers_value(vm):
    ...
    vm.block.header.state_root = vm_state.state_root
    transaction = UnsignedUserAccountTransaction(
        ** merge(DEFAULT_TX_PARAMS, {"nonce": get_nonce(vm), "value": 10})
    ).as_signed_transaction(
        PRIVATE_KEY
    )
    computation, _ = vm.apply_transaction(transaction)
...
def test_call_checks_signature(vm, v, r, s):
    transaction = UserAccountTransaction(
        ** merge(DEFAULT_TX_PARAMS, {"v": v, "r": r, "s": s})
    )
    message_params = {
        "gas": transaction.gas,
        "to": transaction.to,
        ...
        **
        merge(
            DEFAULT_TX_PARAMS,
            {
                ...
                "gas": 1 * 1000 * 1000,
                "access_list": DEFAULT_TX_PARAMS["access_list"] +
                [[GAS_LOGGING_CONTRACT_ADDRESS]]
            },
        )
    ).as_signed_transaction(
        PRIVATE_KEY
