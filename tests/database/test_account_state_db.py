...
from evm.exceptions import (UnannouncedStateAccess, ValidationError)

from evm.db.backends.memory import MemoryDB
from evm.db.state import (MainAccountStateDB, ShardingAccountStateDB)

from evm.constants import (EMPTY_SHA3,)
from evm.utils.state_access_restriction import (
    get_balance_key, get_code_key, get_storage_key
)
...
@pytest.mark.parametrize(
    "state", [MainAccountStateDB(MemoryDB()), ShardingAccountStateDB(MemoryDB())]
)
...
@pytest.mark.parametrize("state", [MainAccountStateDB(MemoryDB())])
...
@pytest.mark.parametrize("state", [ShardingAccountStateDB(MemoryDB())])
