...
from evm.vm.forks.sharding.vmc_handler import (VMC,)
from evm.vm.forks.sharding.vmc_utils import (create_vmc_tx, get_vmc_json)
...
@pytest.fixture
def vmc():
    eth_tester = EthereumTester(backend=PyEVMBackend(), auto_mine_transactions=False)
    provider = EthereumTesterProvider(eth_tester)
    w3 = Web3(provider)
    # setup vmc's web3.eth.contract instance
    vmc_tx = create_vmc_tx(ByzantiumTransaction, get_sharding_config()['GAS_PRICE'])
