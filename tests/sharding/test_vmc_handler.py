...
from evm.vm.forks.sharding.log_handler import (LogHandler,)
from evm.vm.forks.sharding.vmc_handler import (
    NextLogUnavailable, ShardTracker, parse_collation_added_log
)
from evm.vm.forks.sharding.vmc_utils import (create_vmc_tx,)

from tests.sharding.fixtures import (vmc,)  # noqa: F401
...
def is_vmc_deployed(vmc_handler):
    return (
        get_code(vmc_handler, vmc_handler.address) != b''
        and get_nonce(vmc_handler, vmc_handler.vmc_tx_sender_address) != 0
    )


def mk_initiating_transactions(
    sender_privkey, sender_starting_nonce, TransactionClass, gas_price
):
...
def deploy_initiating_contracts(vmc_handler, privkey):
    ...
    txs = mk_initiating_transactions(
        privkey, nonce, ByzantiumTransaction, vmc_handler.config['GAS_PRICE']
    )
    for tx in txs:
...
def add_header_constant_call(vmc_handler, collation_header):
    args = (getattr(collation_header, field[0]) for field in collation_header.fields)
    # transform address from canonical to checksum_address, to comply with web3.py
    args_with_checksum_address = (
        to_checksum_address(item) if is_address(item) else item for item in args
    )
...
@pytest.mark.parametrize(  # noqa: F811
    ...
    (((10, 11, 12, 11, 13, 14, 15, 11, 12, 13, 14, 12, 13, 14, 15, 16, 17, 18, 19, 16), (True, True, True, False, True, True, True, False, False, False, False, False, False, False, False, True, True, True, True, False), (19, 18, 17, 16, 16, 15, 15, 14, 14, 14, 13, 13, 13, 12, 12, 12, 11, 11, 11, 10), (True, True, True, True, False, True, False, True, False, False, True, False, False, True, False, False, True, False, False, True)), ((1, 2, 3, 2, 2, 2), (True, True, True, False, False, False), (3, 2, 2, 2, 2, 1), (True, True, False, False, False, True))),
    # test case in doc.md  # noqa: E501  # noqa: E501
)
def test_shard_tracker_fetch_candidate_head(
    vmc, mock_score, mock_is_new_head, expected_score, expected_is_new_head
):
    shard_id = 0
...
def test_vmc_contract_calls(vmc):  # noqa: F811
    ...
    build_transaction_detail = vmc.mk_build_transaction_detail(nonce=0, gas=10000)
    ...
    with pytest.raises(ValueError):
        build_transaction_detail = vmc.mk_build_transaction_detail(
            nonce=None, gas=10000
        )
    with pytest.raises(ValueError):
        build_transaction_detail = vmc.mk_build_transaction_detail(nonce=0, gas=None)
    # test `mk_contract_tx_detail` ######################################
    tx_detail = vmc.mk_contract_tx_detail(
        sender_address=ZERO_ADDR, gas=vmc.config['DEFAULT_GAS']
    )
    assert 'from' in tx_detail
    assert 'gas' in tx_detail
    with pytest.raises(ValueError):
        tx_detail = vmc.mk_contract_tx_detail(sender_address=ZERO_ADDR, gas=None)
    with pytest.raises(ValueError):
        tx_detail = vmc.mk_contract_tx_detail(
            sender_address=None, gas=vmc.config['DEFAULT_GAS']
        )
    ...
    with pytest.raises(TransactionFailed):
        header_parent_not_added = mk_testing_colhdr(vmc, shard_id, header0_1.hash, 1)
    ...
    logs = vmc.web3.eth.getLogs(
        {
            ...
            "topics": [encode_hex(ShardTracker.COLLATION_ADDED_TOPIC)],
        }
    )
    assert len(logs) == 4
    vmc.tx_to_shard(
        ...
        (
            {
                ...
                'logIndex': 0,
                'transactionIndex': 0,
                'transactionHash': b'\xda\xb8:\xe5\x86\xe9Q\xf2\x9c\xc6<g\x9bl\x84\x85\xf4\x1dh\xce\x8d\xe6\xc0D\xa0*E\xd8m\xd4\x01\xcf',
                'blockHash': b'\x13\xa97d\r\x90t\xe5;\x84\xf9\xe0\xb8\xf2c\x1c}\x88\xbf\x84DN\xa0\x16Q\xd9|\xa1\x00\x91\xc0\xbd',
                'blockNumber': 25,
                'address': '0xf4F1600B0a65995833854738764b50A4DA8d6BE1',
                'data': '',
                'topics': [
                    b'\x95\x86g\xed\xf5J\xea\x9d\xfa[\xee!\xb2\xb4\x9f|\x11D\xe4[\xa0h"\xa3\xa5\x8fc\x90\xa9\xa1\xc5C',
            ...
            },  # noqa: E501
            {
                ...
                'period_start_prevhash': b'4\xc9\x98\xa5\xb82Z\x12v\xf3\x85U\x8a\xae\x7f\\?\x8a@\x02=(\x9f9d\x9d/\xcd\xd7\xd4\x91\x00',
        ...
        (
            {
                ...
                'logIndex': 0,
                'transactionIndex': 0,
                'transactionHash': b'\x16\xc2\x0b\xadZ|\x92l@@\xb1\x15\x93nh\xd6]p\x16\xae\xd5\xe7\x9crKl\x8c\xcf\x06\x9a\xd4\x05',
                'blockHash': b'\x94\\\xce\x19\x01:j\xbb\xf8\xba\x19\xcfv\xc3z3}^\xb6>\xa0\x0e\xf74\xe8A\t\x12p\x9a\xf6V',
                'blockNumber': 30,
                'address': '0xf4F1600B0a65995833854738764b50A4DA8d6BE1',
                'data': '',
                'topics': [
                    b'\x95\x86g\xed\xf5J\xea\x9d\xfa[\xee!\xb2\xb4\x9f|\x11D\xe4[\xa0h"\xa3\xa5\x8fc\x90\xa9\xa1\xc5C',
                    b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00',
                ],
            },  # noqa: E501
            {
                ...
                'period_start_prevhash': b'\x83:8W0\x0f]\xc9\\\xb8\x8d4s\xea1X\xc7\xd3\x86\xac\x057\xd6\x14f/\x9d\xe5\\a\x0c#',
...
def test_parse_collation_added_log(
    log, expected_header_dict, expected_is_new_head, expected_score
):
