...
from trinity.utils.chains import (
    get_default_data_dir, get_database_dir, get_nodekey_path, ChainConfig
)
...
def test_chain_config_explicit_properties():
    chain_config = ChainConfig(
        'muffin', data_dir='./data-dir', nodekey_path='./nodekey'
    )
...
def test_chain_config_nodekey_loading(nodekey_bytes, nodekey_path):
    chain_config = ChainConfig('muffin', nodekey_path=nodekey_path)
...
def test_chain_config_explictely_provided_nodekey(nodekey_bytes, as_bytes):
    chain_config = ChainConfig(
        'muffin', nodekey=nodekey_bytes if as_bytes else keys.PrivateKey(nodekey_bytes)
    )
    assert chain_config.nodekey.to_bytes() == nodekey_bytes
