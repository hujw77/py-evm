...
from typing import (
    Any, cast, Callable, Dict, Generator, List, Optional, Tuple, Type
)  # noqa: F401
...
from p2p.utils import (gen_request_id, get_devp2p_cmd_id, roundup_16, sxor)
from p2p import eth
from p2p import les
from p2p.p2p_proto import (Disconnect, DisconnectReason, Hello, P2PProtocol, Ping, Pong)

from .constants import (
    CONN_IDLE_TIMEOUT, HANDSHAKE_TIMEOUT, HEADER_LEN, MAC_LEN, REPLY_TIMEOUT
)
...
class BasePeer(metaclass=ABCMeta):
    ...
    async def do_p2p_handshake(self):
        ...
        if isinstance(cmd, Disconnect):
            ...
            raise HandshakeFailure(
                "{} disconnected before completing handshake: {}".format(
                    self, msg['reason_name']
                )
            )
    ...
    async def read_sub_proto_msg(
    ...
    ) -> Tuple[protocol.Command, protocol._DecodedMsgType]:
        ...
        return await wait_with_token(
            self.sub_proto_msg_queue.get(), token=combined_token
        )
    ...
    async def read(self, n: int) -> bytes:
        ...
        try:
            return await wait_with_token(
                ...
                timeout=self.conn_idle_timeout,
            )
    ...
    async def run(
        self, finished_callback: Optional[Callable[['BasePeer'], None]] = None
    ) -> None:
        try:
    ...
    def disconnect(self, reason: DisconnectReason) -> None:
        ...
        if not isinstance(reason, DisconnectReason):
            self.logger.debug(
                "Disconnecting from remote peer; reason: %s", reason.value
            )
            raise ValueError(
...
class LESPeer(BasePeer):
    ...
    async def _wait_for_reply(
        self, request_id: int, cancel_token: CancelToken
    ) -> Dict[str, Any]:
        reply = None
        ...
        await wait_with_token(
            got_reply.wait(), token=combined_token, timeout=self.reply_timeout
        )
        return reply

    async def get_block_header_by_hash(
        self, block_hash: bytes, cancel_token: CancelToken
    ) -> BlockHeader:
        ...
        if not reply['headers']:
            raise BlockNotFound(
                "Peer {} has no block with hash {}".format(self, block_hash)
            )
    ...
    async def get_account(
        self, block_hash: bytes, address: bytes, cancel_token: CancelToken
    ) -> Account:
        key = keccak(address)
        proof = await self._get_proof(
            cancel_token, block_hash, account_key=b'', key=key
        )
    ...
    async def _get_proof(
        ...
        request_id = gen_request_id()
        self.sub_proto.send_get_proof(
            block_hash, account_key, key, from_level, request_id
        )
        reply = await self._wait_for_reply(request_id, cancel_token)
        return reply['proof']

    async def get_contract_code(
        self, block_hash: bytes, key: bytes, cancel_token: CancelToken
    ) -> bytes:
        request_id = gen_request_id()
    ...
    async def fetch_headers_starting_at(
        self, start_block: int, cancel_token: CancelToken
    ) -> List[BlockHeader]:
        ...
        self.logger.info(
            "fetched headers from %s to %s",
            reply['headers'][0].block_number,
...
class PeerPool:
    ...
    async def run(self) -> None:
        ...
        while not self.cancel_token.triggered:
            try:
                ...
                self.logger.error(
                    "Unexpected error (%s), restarting", traceback.format_exc()
                )
                await self.stop_all_peers()
            # Wait self._connect_loop_sleep seconds, unless we're asked to stop.
            await asyncio.wait(
                [self.cancel_token.wait()], timeout=self._connect_loop_sleep
            )

    async def stop_all_peers(self) -> None:
        self.logger.info("Stopping all peers ...")
        await asyncio.gather(*[peer.stop() for peer in self.connected_nodes.values()])
    ...
    async def connect(self, remote: Node) -> BasePeer:
        ...
        try:
            ...
            self.logger.debug(
                "Could not complete handshake with %s: %s", remote, repr(e)
            )
        except Exception:
            self.logger.warning(
...
def _test():
    ...
    remote = Node(
        keys.PublicKey(decode_hex(args.remoteid)), Address('127.0.0.1', 30303, 30303)
    )
    ...
    peer = loop.run_until_complete(
        asyncio.wait_for(
            handshake(
                remote, ecies.generate_privkey(), peer_class, chaindb, network_id
            ),
            HANDSHAKE_TIMEOUT,
        )
    )
