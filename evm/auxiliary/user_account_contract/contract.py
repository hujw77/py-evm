from evm.constants import (SECPK1_N, ENTRY_POINT)
...
def generate_account_lll_code(address):
    return [
        ...
        ['if', ['eq', ['calldataload', CALLDATA_FUNCTION_ID], NONCE_GETTER_ID * 2 ** 224], ['seq', ['mstore', 0, ['sload', STORAGE_NONCE]], ['return', 0, 32]]],
        ...
        # data ++ tx-sighash)
        ['mstore', MEMORY_SIGHASH, ['sha3', MEMORY_NONCE, ['sub', ['calldatasize'], CALLDATA_NONCE - 32]]],
        # Verify signature by calling validation code
        ['call', ECRECOVER_GAS, ECRECOVER_ADDRESS, 0, MEMORY_SIGHASH, 128, MEMORY_SENDER, 32],
        # input data (spans sighash and signature)
        # output data (address)
        ['with', 'recovered_address', ['mload', MEMORY_SENDER], ['seq', ['assert', 'recovered_address'], ['assert', ['eq', 'recovered_address', address]]]],
        ...
        ['mstore', MEMORY_RETURNCODE, ['call', ['sub', ['gas'], GAS_RESERVE], ['calldataload', CALLDATA_DESTINATION], ['calldataload', CALLDATA_VALUE], MEMORY_DATA, ['sub', ['calldatasize'], CALLDATA_DATA], 0, 0]],
...
def generate_account_bytecode(validation_code_address):
    code = ACCOUNT_BYTECODE_TEMPLATE.replace(
        ADDRESS_PLACEHOLDER, validation_code_address
    )
