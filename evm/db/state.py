...
from evm.db.immutable import (ImmutableDB,)
from evm.db.tracked import (TrackedDB,)
from evm.rlp.accounts import (Account,)
from evm.validation import (
    validate_is_bytes, validate_uint256, validate_canonical_address
)

from evm.utils.state_access_restriction import (is_accessible,)
...
from evm.utils.state_access_restriction import (
    get_code_key, get_balance_key, get_storage_key
)
...
class MainAccountStateDB(BaseAccountStateDB):
    ...
    def account_is_empty(self, address):
        return not self.account_has_code_or_nonce(address) and self.get_balance(
            address
        ) == 0
        ...
        if self.is_access_restricted:
            if not is_accessible(key, self.access_list):
                raise UnannouncedStateAccess(
                    "Attempted state access outside of access set"
                )
