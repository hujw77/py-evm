...
def calldatacopy(computation):
    (mem_start_position, calldata_start_position, size) = computation.stack.pop(
...
def codecopy(computation):
    (mem_start_position, code_start_position, size) = computation.stack.pop(
        num_items=3, type_hint=constants.UINT256
    )
    computation.extend_memory(mem_start_position, size)
    word_count = ceil32(size) // 32
    copy_gas_cost = constants.GAS_COPY * word_count
    computation.gas_meter.consume_gas(copy_gas_cost, reason="CODECOPY: word gas cost")
...
def extcodecopy(computation):
    ...
    computation.gas_meter.consume_gas(
        copy_gas_cost, reason='EXTCODECOPY: word gas cost'
    )
    with computation.state_db(read_only=True) as state_db:
...
def returndatacopy(computation):
    (mem_start_position, returndata_start_position, size) = computation.stack.pop(
        num_items=3, type_hint=constants.UINT256
    )
