...
class Call(BaseCall):
    ...
    def get_call_params(self, computation):
        ...
        (
            ...
            num_items=5, type_hint=constants.UINT256
        )
        return (
            gas,
            value,
...
class DelegateCall(BaseCall):
    ...
    def get_call_params(self, computation):
        ...
        (
            ...
            num_items=4, type_hint=constants.UINT256
        )
        to = computation.msg.storage_address
    ...
    if computation.gas_meter.gas_remaining < extra_gas:
        ...
        raise OutOfGas(
            "Out of gas: Needed {0} - Remaining {1} - Reason: {2}".format(
                extra_gas, computation.gas_meter.gas_remaining, mnemonic
            )
        )

    gas = min(
        gas, max_child_gas_eip150(computation.gas_meter.gas_remaining - extra_gas)
    )
    total_fee = gas + extra_gas
...
class CallEIP161(CallEIP150):
    ...
    def compute_msg_extra_gas(self, computation, gas, to, value):
        with computation.state_db(read_only=True) as state_db:
            account_is_dead = (
                not state_db.account_exists(to) or state_db.account_is_empty(to)
            )
        transfer_gas_fee = constants.GAS_CALLVALUE if value else 0
