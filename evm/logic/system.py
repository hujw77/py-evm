...
from evm.utils.address import (
    force_bytes_to_address, generate_contract_address, generate_CREATE2_contract_address
)
...
def selfdestruct_eip161(computation):
    ...
    with computation.state_db(read_only=True) as state_db:
        is_dead = (
            not state_db.account_exists(beneficiary)
            or state_db.account_is_empty(beneficiary)
        )
...
class Create2(CreateEIP150):
    ...
    def __call__(self, computation):
        ...
        contract_address = generate_CREATE2_contract_address(salt, call_data)
        with computation.state_db(read_only=True) as state_db:
