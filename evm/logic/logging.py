...
def log_XX(computation, topic_count):
    ...
    mem_start_position, size = computation.stack.pop(
        num_items=2, type_hint=constants.UINT256
    )
    if not topic_count:
        topics = []
    elif topic_count > 1:
        topics = computation.stack.pop(
            num_items=topic_count, type_hint=constants.UINT256
        )
    else:
        topics = [
            computation.stack.pop(num_items=topic_count, type_hint=constants.UINT256)
        ]
    ...
    computation.gas_meter.consume_gas(
        total_gas_cost, reason="Log topic and data gas cost"
    )
