...
def state_definition_to_dict(state_definition):
    ...
    if isinstance(state_definition, Mapping):
        ...
        state_dicts = [
            assoc_in({}, state_item[:-1], state_item[-1]) if not isinstance(
                state_item, Mapping
            ) else state_item
...
normalize_environment = dict_options_normalizer(
    [normalize_main_environment, normalize_sharding_environment]
)
...
normalize_transaction = dict_options_normalizer(
    [normalize_main_transaction, normalize_sharding_transaction]
)
...
normalize_transaction_group = dict_options_normalizer(
    [normalize_main_transaction_group, normalize_sharding_transaction_group]
)
