...
from evm.exceptions import (ContractCreationCollision,)
from evm.rlp.logs import (Log,)
from evm.rlp.receipts import (Receipt,)
from evm.vm.message import (Message,)
from evm.vm_state import (BaseVMState, BaseTransactionExecutor)
...
class FrontierTransactionExecutor(BaseTransactionExecutor):
        ...
        with self.mutable_state_db() as state_db:
            ...
            if transaction.to == constants.CREATE_CONTRACT_ADDRESS:
                contract_address = generate_contract_address(
                    transaction.sender, state_db.get_nonce(transaction.sender) - 1
                )
                data = b''
    ...
    def run_post_computation(self, transaction, computation):
        ...
        self.logger.debug(
            'TRANSACTION FEE: %s -> %s', transaction_fee, encode_hex(self.coinbase)
        )
        with self.mutable_state_db() as state_db:
...
def _make_frontier_receipt(vm_state, transaction, computation):
    ...
    receipt = Receipt(state_root=vm_state.state_root, gas_used=gas_used, logs=logs)
    return receipt
