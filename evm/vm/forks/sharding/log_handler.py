...
def preprocess_block_param(w3, block_param):
    if isinstance(block_param, int):
        ...
        raise ValueError(
            "block parameter {0} must be int or str type".format(block_param)
        )
...
class LogHandler:
    ...
    @to_dict
    def mk_filter_params(
        self, from_block_number, to_block_number, address=None, topics=None
    ):
    ...
    def get_new_logs(self, address=None, topics=None):
        ...
        revoked_hashes, new_block_hashes = check_chain_head(
            self.w3, self.recent_block_hashes, self.history_size
        )
        ...
        filter_params = self.mk_filter_params(
            from_block_number, to_block_number, address, topics
        )
