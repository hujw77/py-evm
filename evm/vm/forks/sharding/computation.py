...
from evm.constants import (STACK_DEPTH_LIMIT,)
from evm.exceptions import (
    OutOfGas, InsufficientFunds, StackDepthLimit, GasPriceAlreadySet, NotTopLevelCall
)
...
from evm.vm.forks.spurious_dragon.constants import (
    EIP170_CODE_SIZE_LIMIT, GAS_CODEDEPOSIT
)
...
class ShardingComputation(ByzantiumComputation):
    ...
    def set_PAYGAS_gasprice(self, gas_price):
        ...
        if self._paygas_gasprice is None:
            ...
            raise GasPriceAlreadySet(
                "PAYGAS is already triggered once with gas price: {}".format(
                    self._paygas_gasprice
                )
            )
    ...
    def apply_message(self):
        ...
        computation = self.apply_computation(
            self.vm_state, self.msg, self.transaction_context
        )
            ...
            if contract_code and len(contract_code) >= EIP170_CODE_SIZE_LIMIT:
                ...
                try:
                    computation.gas_meter.consume_gas(
                        contract_code_gas_cost, reason="Write contract code for CREATE2"
                    )
