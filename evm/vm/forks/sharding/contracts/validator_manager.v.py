...
validators: public(
    {deposit: wei_value, addr: address}[num]
...
# Collation headers
collation_headers: public({parent_hash: bytes32, score: num}[bytes32][num])
# Receipt data
receipts: public(
    {
        shard_id: num,
...
def deposit() -> num:
    ...
    self.validators[index] = {deposit: msg.value, addr: validator_addr}
...
def withdraw(validator_index: num) -> bool:
    ...
    self.validators[validator_index] = {deposit: 0, addr: None}
...
def get_eligible_proposer(shard_id: num, period: num) -> address:
    ...
    return self.validators[
        as_num128(num256_mod(as_num256(sha3(concat(blockhash((period - self.lookahead_periods) * self.period_length), as_bytes32(shard_id)))), as_num256(self.get_validators_max_index())))
...
def add_header(
    ...
    receipt_root: bytes32,
    collation_number: num,
) -> bool:  # TODO: cannot be named `number` since it is reserved
    zero_addr = 0x0000000000000000000000000000000000000000
    ...
    assert period_start_prevhash == blockhash(
        expected_period_number * self.period_length - 1
    )
    ...
    validator_addr = self.get_eligible_proposer(
        shard_id, block.number / self.period_length
    )
    ...
    self.collation_headers[shard_id][entire_header_hash] = {
        parent_hash: parent_hash, score: _score
    }
