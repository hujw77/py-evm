...
from evm.vm.forks.frontier.vm_state import (FrontierTransactionExecutor,)
    ...
    def run_post_computation(self, transaction, computation):
        ...
        with self.mutable_state_db() as state_db:
            for account in touched_accounts:
                if state_db.account_exists(account) and state_db.account_is_empty(
                    account
                ):
                    self.logger.debug("CLEARING EMPTY ACCOUNT: %s", encode_hex(account))
