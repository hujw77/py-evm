...
async def connection_loop(execute_rpc, reader, writer, logger, cancel_token):
    ...
    while True:
        request_bytes = b''
        try:
            request_bytes = await wait_with_token(
                reader.readuntil(b'}'), token=cancel_token
            )
        except asyncio.LimitOverrunError as e:
            logger.info("Client request was too long. Erasing buffer and restarting...")
            request_bytes = await wait_with_token(
                reader.read(e.consumed), token=cancel_token
            )
        ...
        if not request:
            ...
            await wait_with_token(
                write_error(writer, 'Invalid Request: empty'), token=cancel_token
            )
            continue
        ...
        try:
            ...
            await wait_with_token(
                write_error(writer, "unknown failure: " + str(e)), token=cancel_token
            )
        else:
