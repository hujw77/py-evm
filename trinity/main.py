...
from p2p.peer import (LESPeer, HardCodedNodesPeerPool)
...
from trinity.constants import (ROPSTEN, SYNC_LIGHT)
...
from trinity.utils.ipc import (wait_for_ipc, kill_process_gracefully)
from trinity.utils.logging import (setup_trinity_logging, with_queued_logging)
from trinity.utils.mp import (ctx,)
...
def run_networking_process(
    chain_config: ChainConfig, sync_mode: str, pool_class: Type[HardCodedNodesPeerPool]
) -> None:
    ...
    peer_pool = pool_class(
        LESPeer, chaindb, chain_config.network_id, chain_config.nodekey
    )
